...
var trezor = _interopRequireWildcard(require("trezor.js"));
...
list.on('connect', async function (device) {
  ...
  console.log('Connected device ' + device.features.label); // What to do on user interactions:

  device.on('button', function (code) {
    handleButton(device.features.label, code);
  });
  device.on('passphrase', handlePass);
  device.on('pin', handlePin); // For convenience, device emits 'disconnect' event on disconnection.
  ...
  await device.waitForSessionAndRun(async session => {
    const value = await session.cipherKeyValue([(10016 | 0x80000000) >>> 0, 0], 'Unlock encrypted storage?', '2d650551248d792eabf628f451200d7f51cb63e46aadcbb1038aacb05e8c8aee2d650551248d792eabf628f451200d7f51cb63e46aadcbb1038aacb05e8c8aee', true, true, true);
    console.log(value);
  }).catch(console.log);
